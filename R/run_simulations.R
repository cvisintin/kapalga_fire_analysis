...
# Write out raw data
save(sim_pops_no_quolls, file = "output/all_pops_all_scenarios_no_quolls")
...
# Write out raw data
save(sim_pops_quolls, file = "output/all_pops_all_scenarios_quolls")

# Extract final populations for all replicates and scenarios
final_pops_quolls <- lapply(seq_len(length(quoll_indices)),
                               function(x) data.frame(rep(paste(params[quoll_indices[x] , 1], params[quoll_indices[x] , 2], "disp", params[quoll_indices[x] , 3], "fires", sep = "_"),
...
write.csv(rbind(final_pops_no_quolls, final_pops_quolls), file = "output/final_pop_size_table.csv", row.names = FALSE)
